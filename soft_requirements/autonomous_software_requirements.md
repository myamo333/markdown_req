...
| ID      | 要件内容                                                                         | 応答時間 |
| ------- | -------------------------------------------------------------------------------- | -------- |
| SWC-201 | 縦制御ループは20ms周期で実行し、コマンドと実績との差が5%を超えた場合に補正を行う | 20ms     |
| SWC-202 | 横制御はStanley法を基礎とし、カーブ半径が150m未満の場合は舵角制限を強化する      | 20ms     |
...
| テストカテゴリ | 内容                                       | 成功条件                                 |
| -------------- | ------------------------------------------ | ---------------------------------------- |
| ユニットテスト | 知覚・計画・制御各モジュールのユニット実行 | カバレッジ80%以上、クリティカル関数100%  |
| SIL            | シナリオシミュレーション（天候/交通条件）  | 1,000ケース連続稼働でフェイルなし        |
| HIL            | 実ECUと動的ベンチで計画～制御ループを検証  | 制御遅延が要求値以内、フォールバック成功 |
